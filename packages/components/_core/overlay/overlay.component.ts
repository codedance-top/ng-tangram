...
import { OVERLAY_POSITIONS, NtOverlayPosition, getPositionNameOfKebabCase } from './overlay-positions';
...
export class NtOverlayComponent {
  ...
  private _paddingClass = getPositionNameOfKebabCase(this._positionPairs[0]);
  ...
  private _setPosition() {
    ...
    this._paddingClass = getPositionNameOfKebabCase(this._positionPairs[0]);
  }
  ...
  onOverlayPositionChange(event: ConnectedOverlayPositionChange) {
    this._positionChange.next(getPositionNameOfKebabCase(event.connectionPair));
  }
